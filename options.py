...
def get_options(option_type):
  ...
  tf.app.flags.DEFINE_integer("segnet", 0, "use segmentation mode: 0 - no, 1 - encoder only")
  ...
  if option_type == 'training':
    ...
    tf.app.flags.DEFINE_integer("local_t_max", 10, "repeat step size")
    ...
    tf.app.flags.DEFINE_integer("experience_history_size", 25, "experience replay buffer size") # 2000
    tf.app.flags.DEFINE_integer("max_time_step", int(13.2 * 10**6), "max time steps")
    tf.app.flags.DEFINE_integer("save_interval_step", 10 * 10, "saving interval steps") # 100*1000
